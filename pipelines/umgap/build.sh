...
# For each database type, it attempts to download, decompress, and convert it  #
# to a tabular format, storing the result in the uniprot_entries.tsv.lz4 file  #
# as a result. Also generate a sequences table containing in-silico            #
# tryptically digested peptides. The function also sorts and compresses the    #
# peptide output.                                                              #
#                                                                              #
# Globals:                                                                     #
#   CURRENT_LOCATION   - Current script directory                              #
...
download_and_process_uniprot_tryptic() {
  ...
  download_uniprot "$db_types" \
  | "$RUST_BIN_DIR"/uniprot-parser-tryptic \
...
calculate_equalized_lcas() {
  ...
  local output_dir="$3"
	...
	$CMD_LZ4CAT "$temp_dir/$temp_constant/peptides_by_equalized.tsv.lz4" | cut -f 2,6 \
		| "$RUST_BIN_DIR"/lca-calculator --input-file "$(luz "$output_dir/lineages.tsv.lz4")" \
...
calculate_equalized_fas() {
  ...
  local temp_constant="$2"
	...
	"$RUST_BIN_DIR"/function-calculator --input-file "peptides_eq" > "$(lz "$temp_dir/$temp_constant/FAs_equalized.tsv.lz4")"
	rm "peptides_eq"
...
calculate_original_fas() {
  ...
  local temp_constant="$2"
	...
	"$RUST_BIN_DIR"/function-calculator --input-file "peptides_orig" > "$(lz "$temp_dir/$temp_constant/FAs_original.tsv.lz4")"
	rm "peptides_orig"
