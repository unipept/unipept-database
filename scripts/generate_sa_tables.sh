...
# Flag indicating whether to only use UniProt entries that are part of a valid reference proteome
PROTEOMES_ONLY="false"
...
print_help() {
  ...
  echo "  --proteomes-only    Only use UniProt entries that are part of a valid reference proteome (optional, default: 'false')."
...
download_and_process_uniprot "$DB_TYPES" "$TEMP_DIR" "$UNIPEPT_TEMP_CONSTANT" "$OUTPUT_DIR" "$PROTEOMES_ONLY"
