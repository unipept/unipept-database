...
gzip -c "${FIXTURES}/uniprot_sprot.dat" > "${WORK}/uniprot_sprot.dat.gz"
(cd "${FIXTURES}" && zip -q "${WORK}/taxdmp.zip" names.dmp nodes.dmp)
...
check "ec_numbers.tsv" "$(table ec_numbers)" "$(printf '%s\n' \
    ...
    $'4\t2.7.11.1\tNon-specific serine/threonine protein kinase')"
