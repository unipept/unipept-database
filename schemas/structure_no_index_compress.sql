...
--
-- Using a view to decompress data
--
-- BLOCKSIZE 16 IS DEFAULT BLOCKSIZE
-- Only BLOBS will get compressed
-- -----------------------------------------------------
CREATE  TABLE IF NOT EXISTS `unipept`.`sequences_compressed` (
  `id` INT UNSIGNED NOT NULL AUTO_INCREMENT ,
...
VIEW `unipept`.`sequences`
AS select
    `unipept`.`sequences_compressed`.`id` AS `id`,
    `unipept`.`sequences_compressed`.`sequence` AS `sequence`,
    `unipept`.`sequences_compressed`.`lca` AS `lca`,
    `unipept`.`sequences_compressed`.`lca_il` AS `lca_il`,
    uncompress(`unipept`.`sequences_compressed`.`fa`) as `fa`,
    uncompress(`unipept`.`sequences_compressed`.`fa_il`) as `fa_il`
from
